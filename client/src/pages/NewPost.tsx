...
export default function NewPost() {
  ...
  const createPost = trpc.community.create.useMutation({ onSuccess: async created => { try { await updatePreferences.mutateAsync({ displayNamePreference: values.displayName, availabilityPreference: values.availability }); } catch { /* The post itself is already saved; preferences can be updated on the next post. */ } if (shouldCelebrate(celebrationTriggerForPostKind(postKind))) { sonnerToast.success("Thank you for showing up.", { description: "Your offer is now on the neighborhood board." }); setCelebrating(true); await new Promise(resolve => window.setTimeout(resolve, 650)); } await refreshAfterPostCreation({ refreshBoard: () => utils.community.list.invalidate(), refreshProfile: () => utils.community.mine.invalidate(), refreshDetail: postId => utils.community.get.invalidate({ id: postId }), refreshCandidates: () => utils.matching.rankForPost.invalidate(), refreshMatches: () => utils.matching.forPost.invalidate() }, created.id); setLocation(`/help/${created.id}`); }, onError: error => setFormError(error.message || "We could not save this post. Please try again.") });
